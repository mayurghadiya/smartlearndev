...
$(document).on('keypress', '.chat-textarea input', function(e){
        ...
        if(message !== "" && e.which == 13){
            txtarea.val('');
            // save the message 
            $.ajax({ type: "POST", url: base  + "student/save_message", data: {message: message, user : user},cache: false,
                     ...
                     $.each(thread, function() {
                        if($("#chat-box").is(":visible")){
                            chatbuddy = $("#chat_buddy_id").val();
                                ...
                                $.ajax({ type: "POST", url: base + "student/mark_read", data: {id: this.msg}});
                                }
                                else{
                                    from = this.sender;
...
function load_thread(user, limit){
       ...
       $.ajax({ type: "POST", url: base  + "student/messages", data: {user : user, limit:limit },cache: false,
        success: function(response){
        if(response.success){
