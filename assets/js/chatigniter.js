...
$(document).on('keypress', '.chat-textarea input', function(e){
        ...
        if(message !== "" && e.which == 13){
            txtarea.val('');
            // save the message 
            $.ajax({ type: "POST", url: base  + "admin/save_message", data: {message: message, user : user},cache: false,
                        ...
                        if($("#chat-box").is(":visible")){
                            chatbuddy = $("#chat_buddy_id").val();
                                if(this.sender == chatbuddy){
									var img_src = base_img+'uploads/student_image/'+this.avatar;
                                ...
                                $.ajax({ type: "POST", url: base + "admin/mark_read", data: {id: this.msg}});
                                }
                                else{
                                    from = this.sender;
...
function load_thread(user, limit){
       ...
       $.ajax({ type: "POST", url: base  + "admin/messages", data: {user : user, limit:limit },cache: false,
        success: function(response){
        if(response.success){
