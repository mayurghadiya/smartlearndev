...
if ((string)$Params['user_parameters_unordered']['action'] == 'statusdb' || (string)$Params['user_parameters_unordered']['action'] == 'statusdbdoupdate') {
	...
	if (erConfigClassLhConfig::getInstance()->getSetting( 'site', 'debug_output' ) == false) {
	   $contentData = erLhcoreClassModelChatOnlineUser::executeRequest('https://##livehelperchat/master/lhc_web/doc/update_db/structure.json');
	} else {
...
array('title' => erTranslationClassLhTranslation::getInstance()->getTranslation('system/update',' update information')));
