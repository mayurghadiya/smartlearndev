                            ...
                            <div class="col-sm-7">
                                <select class="form-control" id="edit_exam_list" name="exam_list">
                                    ...
                                    <?php foreach ($exam_list as $list) { ?>
                                        <option value="<?php echo $list->em_id; ?>"
                                                <?php if ($list->em_id == $edit_data->exam_ref_id) echo 'selected'; ?>><?php echo $list->em_name; ?></option>
                                            <?php } ?>
                                </select>
                                <input type="hidden" name="exam_name" id="edit_exam_name" value=""/>
                            </div>
                        </div>
                        <div class="form-group">
...
<script>
    $(document).ready(function () {
        ...
        $('#edit_semester').on('change', function () {
            var course_id = $('#edit_degree').val();
            var branch_id = $('#edit_course').val();
        ...
        function get_exam_list(course, branch, batch, semester) {
            $.ajax({
                url: '<?php echo base_url(); ?>index.php?admin/get_exam_list_data/' + course + '/' + branch + '/' + batch + '/' + semester + '/remedial',
                type: 'get',
                success: function (content) {
                    console.log(content);
                    $('#edit_exam_list').find('option').remove().end();
                    var exam_list = jQuery.parseJSON(content);
                    $('#edit_exam_list').append('<option value="">Select</option>');
                    $.each(exam_list, function (key, value) {
                        $('#edit_exam_list').append('<option value=' + value.em_id + '>' + value.em_name + '</option>');
                    });
                }
            });
        }
        ...
        function exam_details(exam_id) {
            ...
            $.ajax({
                ...
                success: function (content) {
                    ...
                    if (exam_details.length == 1) {
                        $('#edit_total_marks').val(exam_details[0].total_marks);
                        $('#edit_passing_marks').val(exam_details[0].passing_mark);
                        exam_types(exam_details[0].em_type);
                    } else {
                        $('#edit_total_marks').val('');
                        $('#edit_passing_marks').val('');
                    }
                }
            });
        }
        ...
        $('#edit_exam_name').val($('#edit_exam_list option:selected').text());

        $('#edit_exam_list').on('change', function () {
            var exam_id = $(this).val();
            var exam_name = $('option:selected', this).text();
            $('#edit_exam_name').val(exam_name);
