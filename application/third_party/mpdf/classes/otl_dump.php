					...
					if ($Lookup[$luli]['Type'] == 1) {
						...
						if ($PosFormat==1) {
							...
							for($g=0;$g<count($glyphs);$g++) {
								...
								$html .= '&nbsp; &raquo; &raquo; &nbsp;';
								if ($level==2 && $exB) { $html .= $exB; }
								$html .= '<span class="changed" style="font-feature-system_setting:\''.$tag.'\' 1;">&nbsp;'.$this->formatEntity($glyphs[$g]).'</span>';
								if ($level==2 && $exL) { $html .= $exL; }
...
/*
									...
									$html .= '&nbsp; &raquo; &raquo; &nbsp;';
									if ($level==2 && $exB) { $html .= $exB; }
									$html .= '<span class="changed" style="font-feature-system_setting:\''.$tag.'\' 1;">&nbsp;'.$this->formatEntity($glyphs[$p]).$this->formatEntity($SecondGlyph).'</span>';
									if ($level==2 && $exL) { $html .= $exL; }
									$html .= ' <span class="unicode">';
...
$html .= '</div>';
									...
									for($j=0;$j<$Class2Count;$j++) {
											...
											for($c1=0;$c1<count($Class1[$i]);$c1++) {
												...
												for($c2=0;$c2<count($Class2[$j]);$c2++) {
													...
													$html .= '&nbsp; &raquo; &raquo; &nbsp;';
													if ($level==2 && $exB) { $html .= $exB; }
													$html .= '<span class="changed" style="font-feature-system_setting:\''.$tag.'\' 1;">&nbsp;'.$this->formatEntity($FirstGlyph).$this->formatEntity($SecondGlyph).'</span>';
													if ($level==2 && $exL) { $html .= $exL; }
													$html .= ' <span class="unicode">';
													if ($Value1['XPlacement']) { $html .= ' Xpl[1]: '.$Value1['XPlacement'].';'; }
													if ($Value1['YPlacement']) { $html .= ' YPl[1]: '.$Value1['YPlacement'].';'; }
					...
					////////////////////////////////////////////////////////////////////////////////
						...
						// Example
						$html .= '<div class="glyphs" style="font-feature-system_setting:\''.$tag.'\' 1;">Example(s): ';
...
*/
					...
					////////////////////////////////////////////////////////////////////////////////
						...
						if ($firstMark) { 
							...
							// Example
							$html .= '<div class="glyphs" style="font-feature-system_setting:\''.$tag.'\' 1;">Example(s): <span class="changed">';
