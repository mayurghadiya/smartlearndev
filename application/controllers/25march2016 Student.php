...

{
...

    {
...

		{
			...
			//$this->load->view('backend/index/gadgets', $page_data);
		...
		//echo $this->db->last_query();
			...
			if(count($events) > 0){ 
				...
				foreach ($events as $erow) {
					...
					$nnewDate = date("d-m-Y", strtotime($erow['event_date']));
	...
	{   
		...
		$this->load->view('backend/index'.$page, $data);
...
function exam_center($param1 = '', $param2 = '') {
...

		{
			...
			if ($param1 == 'online') {			
				...
				$page_data['page_title'] = 'Online Assignment';
			...
			if ($param1 == 'offline') {			
				...
				$page_data['page_title'] = 'Offline Assignment';
				
				$this->load->view('backend/index', $page_data);

			}

			if ($param1 == 'completed_assignment') {

				

				$page_data['assignment']    = $this->db->get_where('assignment_manager',array("student_id"=>$this->session->userdata('std_id')))->result();
				...
				$page_data['page_title'] = 'Completed Assignment';
				
				$this->load->view('backend/index', $page_data);

			}

			if ($param1 == 'create_online_assignment') {
				...
				$data['assign_url'] = $this->input->post('assignmenturl');
				...
				$data['assign_online_status'] = 'online';						
				...
				redirect(base_url() . 'index.php?student/assignment/completed_assignment', 'refresh');

			}

			if ($param1 == 'create_offline_assignment') {
				...
				$data['assign_desc'] = $this->input->post('content_data');

				$data['assign_dos'] = $this->input->post('submissiondate');

				$data['assign_status'] = 1;			

				$data['assign_online_status'] = 'offline';						
...

		{
			...
			if ($param1 == 'create') {
				...
				$data['pm_student_id'] = $this->session->userdata('student_id');		
...

			{
				...
				$page_data['page_title'] = 'Project Submission';
	...
	function participate($param1 = '', $param2 = '')
		{
			if($param1=="create")
			{
				...
				$data['contactus']  = $this->input->post('contact');				
