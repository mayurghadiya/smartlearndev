...
class Sms_model extends CI_Model {
    ...
    {
        $active_sms_service = $this->db->get_where('system_setting' , array(
    ...
    function send_sms_via_clickatell($message = '' , $reciever_phone = '') {
        ...
        $clickatell_user       = $this->db->get_where('system_setting', array('type' => 'clickatell_user'))->row()->description;
        $clickatell_password   = $this->db->get_where('system_setting', array('type' => 'clickatell_password'))->row()->description;
        $clickatell_api_id     = $this->db->get_where('system_setting', array('type' => 'clickatell_api_id'))->row()->description;
    ...
    function send_sms_via_twilio($message = '' , $reciever_phone = '') {
        ...
        $account_sid    = $this->db->get_where('system_setting', array('type' => 'twilio_account_sid'))->row()->description;
        $auth_token     = $this->db->get_where('system_setting', array('type' => 'twilio_auth_token'))->row()->description;
        $client         = new Services_Twilio($account_sid, $auth_token); 

        $client->account->messages->create(array( 
            'To'        => $reciever_phone, 
            'From'      => $this->db->get_where('system_setting', array('type' => 'twilio_sender_phone_number'))->row()->description,
            'Body'      => $message   
        ));
